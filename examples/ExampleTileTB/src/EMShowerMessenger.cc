...
{
  fDirectory = new G4UIdirectory("/exampleTileTB/fastSim/");
  fDirectory->SetGuidance("Set mesh parameters for the example fast sim model.");

  fPrintCmd = new G4UIcmdWithoutParameter("/exampleTileTB/fastSim/print", this);
  fPrintCmd->SetGuidance("Print current settings.");

  fNbOfHitsCmd = new G4UIcmdWithAnInteger("/exampleTileTB/fastSim/numberOfHits", this);
