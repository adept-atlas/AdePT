...
{
  fExampleDir = new G4UIdirectory("/exampleTileTB/");
  fExampleDir->SetGuidance("UI commands specific to this example");

  fDetectorDir = new G4UIdirectory("/exampleTileTB/detector/");
  fDetectorDir->SetGuidance("Detector construction UI commands");

  fAdeptDir = new G4UIdirectory("/exampleTileTB/adept/");
  fDetectorDir->SetGuidance("AdePT integration UI commands");

  fPrintCmd = new G4UIcmdWithoutParameter("/exampleTileTB/detector/print", this);
  fPrintCmd->SetGuidance("Print current settings.");

  fFileNameCmd = new G4UIcmdWithAString("/exampleTileTB/detector/filename", this);
  fFileNameCmd->SetGuidance("Set GDML file name");
  //
  fRegionNameCmd = new G4UIcmdWithAString("/exampleTileTB/detector/regionname", this);
  fRegionNameCmd->SetGuidance("Set fast simulation region name");
  //
  fSensVolNameCmd = new G4UIcmdWithAString("/exampleTileTB/detector/addsensitivevolume", this);
  fSensVolNameCmd->SetGuidance("Add a sensitive volume to the list");
  //
  fSensVolGroupCmd = new G4UIcmdWithAString("/exampleTileTB/detector/sensitivegroup", this);
  fSensVolGroupCmd->SetGuidance("Define a wildcard for a sensitive volumes group");
  //
  fActivationCmd = new G4UIcmdWithABool("/exampleTileTB/adept/activate", this);
  fActivationCmd->SetGuidance("(Activate AdePT");
  //
  fVerbosityCmd = new G4UIcmdWithAnInteger("/exampleTileTB/adept/verbose", this);
  fVerbosityCmd->SetGuidance("Verbosity level for AdePT integration transport");

  fBufferThresholdCmd = new G4UIcmdWithAnInteger("/exampleTileTB/adept/threshold", this);
  fBufferThresholdCmd->SetGuidance("Threshold for starting AdePT transport");

  fTrackSlotsCmd = new G4UIcmdWithAnInteger("/exampleTileTB/adept/milliontrackslots", this);
  fTrackSlotsCmd->SetGuidance("Total number of allocated track slots per GPU");

  fFieldCmd = new G4UIcmdWith3VectorAndUnit("/exampleTileTB/detector/setField", this);
